...
const Popup: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    chrome.storage.session.get(["allowedDomains"], (result: any) => {
      const allowed = result?.allowedDomains ?? ["thehill.com", "theroot.com", "usanews.com"];
      setDomains(allowed);
    });
  }, []);
  ...
  return (
    <div role="region" aria-label="Bluesky Link Card popup" className="w-80 p-4">
      ...
      {loggedIn ? (
        <div className="space-y-1 mb-4">
          <div>Logged in as <span className="font-medium">@{identifier}</span></div>
        </div>
      ) : (
...
describe("Popup", () => {
  beforeEach(() => {
    ...
    vi.mocked(chrome.storage.session.get).mockImplementation((_, callback) => {
      callback({ allowedDomains: ["thehill.com", "theroot.com", "usanews.com"] });
    });
  });
  ...
  it("should render login form initially", () => {
    ...
    expect(screen.getByRole("button", { name: /sign in to bluesky/i })).toBeInTheDocument();
  });
  ...
  it("should handle login", async () => {
    ...
    const loginButton = screen.getByRole("button", { name: /sign in to bluesky/i });
  ...
  it("should have accessible aria labels", () => {
    ...
    expect(screen.getByRole("region", { name: /bluesky link card popup/i })).toBeInTheDocument();
    expect(screen.getByRole("list", { name: /allowed domains/i })).toBeInTheDocument();
  });
